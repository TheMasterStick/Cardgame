...
```json
{
  ...
  "specializations": [
    { "id": "arcane-efficiency", "name": "Arcane Efficiency", "text": "Your first Spell each turn costs 1 less Mana.", "effect": { "kind": "firstSpellDiscount", "amount": 1 } },
    { "id": "battle-conjuration", "name": "Battle Conjuration", "text": "Your creatures have +1 Attack.", "effect": { "kind": "auraBuff", "filter": "all", "attackDelta": 1 } },
    { "id": "warding-sigils", "name": "Warding Sigils", "text": "Your creatures have +1 Health.", "effect": { "kind": "auraBuff", "filter": "all", "hpDelta": 1 } }
  ],
...
- `specializations`: **required**, exactly 3 `HeroSpecialization` entries
  (Phase P, DESIGN.md §19) — `{ "id": string, "name": string, "text":
  string, "effect": PassiveEffect }`. `effect` is an always-on
  `PassiveEffect`, from a small curated template set (see DESIGN.md §9):
  - `{ "kind": "auraBuff", "filter": "all" | { "race": Race } | { "faction": Faction }, "attackDelta"?: N, "hpDelta"?: N }` — live bonus to every matching friendly creature, recomputed on every Attack/effective-max-HP read (same "never stored" approach as Flank/Formation); at least one of `attackDelta`/`hpDelta` should be set.
  ...
  - The player picks one of the three **per match** (not at deck-build time, not baked into the card) — the fixed `heroPower`/optional `signature` below are unaffected by which one is chosen. `id` only needs to be unique within that Hero's own 3-entry list.
- `heroPower` *(optional)*: `{ "effect": CardEffect, "activateCost": N, "text"?: string }` — Energy-costed, usable once per turn (resets every `startTurn`), same `CardEffect` shape and `resolveEffect` machinery as a Spell/Ability. Fixed regardless of which Specialization is chosen.
